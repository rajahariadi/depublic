...
@section('content')
    <div class="card">
        <div class="card-body">
            <div>
                <h3 class="card-title"><i class="lni lni-star"></i></i> Edit Event</h3>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <form action="{{ route('admin.events.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf @method('PUT')
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="defaultFormControlInput" placeholder="Event"
                                aria-describedby="defaultFormControlHelp" name="name" value="{{ $data->name }}">
                        </div>
                        @error('name')
                        ...
                        <select class="single-select" name="event_category_id">
                            @foreach ($event_categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $data->event_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="defaultFormControlInput" placeholder="Location"
                                aria-describedby="defaultFormControlHelp" name="location" value="{{ $data->location }}">
                        </div>
                        @error('location')
                            <div class="mt-2 text-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="input-group">
                            <input type="date" class="form-control" id="defaultFormControlInput"
                                aria-describedby="defaultFormControlHelp" name="start_event" value="{{$data->start_event}}">
                        </div>
                        @error('start_event')
                            <div class="mt-2 text-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="input-group">
                            <input type="date" class="form-control" id="defaultFormControlInput"
                                aria-describedby="defaultFormControlHelp" name="end_event" value="{{$data->end_event}}">
                        </div>
                        @error('end_event')
                            <div class="mt-2 text-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="input-group">
                            <textarea name="highlight" class="form-control" placeholder="Highlight" id="defaultFormControlInput" cols="150"
                                rows="3">{{ $data->highlight }}</textarea>
                        </div>
                        @error('highlight')
                            <div class="mt-2 text-danger">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="text-center">
                        <br><br><br>
                        <img class="text-center" id="gambarBukuPreview" src="{{ Storage::url($data->image) }}"
